...
class Model(object):
    ...
    def predict(self, features, clf_weight, add_seq_features=True, add_poly_features=False,
        ...
        for i in range(1, 4):
            ...
            xgb_list = []
            for file in glob('xgb_models_' + str(i) + '*' + ".joblib.gz"):
                xgb_list = xgb_list + (joblib.load(file))
            cat_tuned_list = []
            for file in glob('cat_tuned_models_' + str(i) + '*' + ".joblib.gz"):
                cat_tuned_list = cat_tuned_list + (joblib.load(file))
        ...
        for i in range(1, 4):
            ...
            if i == 1:
                ...
                best_tuned_cat = {'custom_metric': 'TotalF1:use_weights=True', 'depth': 12,
                                  'fold_len_multiplier': 1.6130787487267035,
                                  'l2_leaf_reg': 3, 'learning_rate': 0.24994004252408897, 'loss_function': 'MultiClass',
                                  'od_type':
                                      'Iter', 'od_wait': 25, 'one_hot_max_size': 10, 'task_type': 'GPU', 'verbose': 500,
                                  'random_seed': 42}
            ...
            for j in range(1, seed_xgb+1):
                ...
                if j%5==0:
                    joblib.dump(xgb_list, "xgb_models_" + str(i) + "_"+ str(version)+ ".joblib.gz", compress=('gzip', 5))
                    version = version+1
                    xgb_list = []
            ...
            for p in range(1, seed_tuned_cat + 1):
                ...
                best_tuned_cat['random_seed'] = 100 + p
                ...
                print('Training seed {} using tuned catboost with input shape {}, it took {:05.2f} seconds {}'.format(
                    100 + p, X_train_cat.shape, (time.time() - start_time), '-' * 20))
                if p % 5 == 0:
...
if __name__ == '__main__':
    ...
    seed_xgb = 10
    seed_tuned_cat = 10
    clf_weight = [1]*seed_xgb + [3]*seed_tuned_cat
